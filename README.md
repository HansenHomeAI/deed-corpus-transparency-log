...
receipt SHA-256 commitments. Each receipt contains a workflow-random 256-bit
binding nonce that remains encrypted, so the public commitment cannot be used
as an offline dictionary oracle for private registry identities. The protected
producer hashes one exact pretty-printed retained byte serialization; the
decryptor verifies that hash before parsing and writes those same bytes. Thus a
retained receipt cannot be independently rewritten through whitespace,
duplicate keys, escape variants, numeric variants, or parser differentials.
